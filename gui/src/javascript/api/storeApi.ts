...
export function createStoreApi(
  ...
  options: CreateStoreApiOptions = {},
) {
  const auditBaseUrl = apiConfig.auditBaseUrl;
  let baseUrl =
    options.baseUrlOverride ??
    apiConfig.storeBaseUrl ??
    (options.storeLocator
      ? computeStoreBaseUrl(options.storeLocator)
      : undefined);
  ...
  return {
    ...
    async listChildren(
      ...
      limit = 20,
      offset = 0,
    ): Promise<CollectionDto<Node>> {
      if (!baseUrl) throw new Error("Store base URL is not configured");
      // debug
      // eslint-disable-next-line no-console
      console.debug(
        "[storeApi] listChildren",
        id,
        "limit=",
        limit,
        "offset=",
    ...
    async getPath(id: string): Promise<Node[]> {
      ...
      const res = await fetchWithAuth(
        tokenProvider,
        `/api/nodes/${encodeURIComponent(id)}/path`,
        { method: "GET" },
        baseUrl,
      );
    ...
    async content(id: string, download = false): Promise<Response> {
      ...
      const res = await fetchWithAuth(
        tokenProvider,
        `/api/nodes/${encodeURIComponent(id)}/content${q}`,
        { method: "GET" },
        baseUrl,
      );
      return res;
    ...
    async create(
      ...
      if (file) {
        ...
        const res = await fetchWithAuth(
          tokenProvider,
          `/api/nodes/${encodeURIComponent(parentId)}`,
          { method: "POST", body: fd },
          baseUrl,
        );
        if (!res.ok) {
    ...
    async listAudits(limit = 100): Promise<Array<AuditEvent>> {
      if (!auditBaseUrl) {
        throw new Error("Audit base URL is not configured");
      }
